...
'if pcPageId<>"" then
	...
	If InStr(Request.ServerVariables("HTTP_X_REWRITE_URL"), "/pages/") = 0 then
		daBlogUrl = True
	Else
		daBlogUrl = False
	End if
	...
	If rstemp("pcCont_Blog") = 0 then
		...
		if daBlogUrl = True then
			...
			daCorrectUrl = Replace(Request.ServerVariables("HTTP_X_REWRITE_URL"),"/blog/","/pages/")
	...
	Else
		...
		if daBlogUrl = False then
			'Perform 301 redirect
			daCorrectUrl = Replace(Request.ServerVariables("HTTP_X_REWRITE_URL"),"/pages/","/blog/")
			Response.Status="301 Moved Permanently"
			Response.AddHeader "Location",daCorrectUrl
		end if
	End If
